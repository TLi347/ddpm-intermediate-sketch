...
from ldm.models.autoencoder import VQModelInterface, IdentityFirstStage, AutoencoderKL, pixel_classifier
...
class LatentDiffusion(DDPM):
    ...
    def __init__(self,
                 ...
                 scale_factor=1.0,
                 scale_by_std=False,
                 unet_trainable="feature_extractor",
                 *args, **kwargs):
    ...
    def shared_step(self, batch, **kwargs):
        x, cond_x, c = self.get_input(batch, self.first_stage_key)
        kwargs = dict()
        kwargs['cond_x'] = cond_x
        ...
        for i in iterator:
            ...
            if self.shorten_cond_schedule:
                ...
                img = self.p_sample(img, cond, ts,
                                    clip_denoised=self.clip_denoised,
                                    score_corrector=kwargs["cond_fn"],
                                    quantize_denoised=quantize_denoised,
                                    corrector_kwargs=kwargs['cond_x'])
            else:
                img = self.p_sample(img, cond, ts,
                                    clip_denoised=self.clip_denoised,
                                    quantize_denoised=quantize_denoised)
    ...
    @torch.no_grad()
    def log_images(self, batch, N=8, n_row=4, sample=True, ddim_steps=None, ddim_eta=1., return_keys=None,
        ...
        log = dict()
        # m = self.get_input(batch, self.first_stage_key,
    ...
    def configure_optimizers(self):
        ...
        if self.unet_trainable == "attn":
            ...
            params = list(self.model.parameters())
        elif self.unet_trainable == "feature_extractor":
            print("Traing the Feature Extractor")
...
class DiffusionWrapper(pl.LightningModule):
    ...
    def forward(self, x, t, c_concat: list = None, c_crossattn: list = None, c_adm=None, **kwargs):
        if self.conditioning_key is None:
            ...
            #     self.diffusion_model(noisy_x, self.diffusion._scale_timesteps(t))
